#!/usr/bin/env bash
# Sync this project from the local workstation to the DAS-5 login node.
#
...
# so you can branch/pull on DAS-5 — drop the `--include='.git/'` line if
# you would rather not ship history.
#
# Usage:
#   bash scripts/sync-local-das5.sh              # dry run (preview only)
#   bash scripts/sync-local-das5.sh --apply      # actually copy
#
# Override the destination with:
#   REMOTE_HOST=das5 REMOTE_DIR='~/path/on/remote' \
#       bash scripts/sync-local-das5.sh --apply

set -euo pipefail

REMOTE_HOST="${REMOTE_HOST:-das5}"
REMOTE_DIR="${REMOTE_DIR:-~/work-my-projects/workspace-master.course.block05-ML4PhA/learning_lbm_collision_operator-WT-01}"
